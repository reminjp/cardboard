...
import { convertGoogleSheetToObjectArray, getGoogleSheet, getGoogleSheetNameById, initializeGoogle, } from './infrastructures/google.js';
import { projectSchema } from './schemas.js';
export async function runBuild(projectDirectoryPath, isPrintAndPlay) {
    /*
     * Read source files.
     */
    const projectDirectoryAbsolutePath = projectDirectoryPath
        ...
        : process.cwd();
    const projectFileAbsolutePath = path.join(projectDirectoryAbsolutePath, 'cardboard.toml');
    const resolvePathInProjectFile = (value) => path.resolve(projectDirectoryAbsolutePath, value);
    const project = projectSchema.parse(parseToml(await fs.readFile(projectFileAbsolutePath, { encoding: 'utf-8' })));
    ...
    for (const tableConfig of project.tables) {
        ...
        if (tableConfig.type === 'google_sheets') {
            ...
            const sheetName = await getGoogleSheetNameById(tableConfig.spreadsheet_id, tableConfig.sheet_id);
        ...
        if (tableConfig.include_record_if) {
            const f = new Function('record', `return ${tableConfig.include_record_if};`);
    ...
    for (const templateConfig of project.templates) {
        const templatejsxAbsolutePath = resolvePathInProjectFile(templateConfig.path);
    ...
    for (const targetConfig of project.targets) {
        ...
        if (isPrintAndPlay && targetConfig.print_and_play?.repeat_record_for) {
            const f = new Function('record', `return ${targetConfig.print_and_play.repeat_record_for};`);
         ...
         * Write PDF.
         */
        const pdfFileName = `${targetConfig.name}${isPrintAndPlay ? '_pnp' : ''}.pdf`;
        await fs.mkdir(resolvePathInProjectFile('build'), { recursive: true });
        await fs.writeFile(resolvePathInProjectFile(`build/${pdfFileName}`), pdfBytes);
    }
}
