...
const PRINT_AND_PLAY_TRIM_MARK_LENGTH_PT = new Length('mm', 1.5).toUnit('pt').number;
const TRIM_MARK_WIDTH_PT = 0.3;
export async function postProcess(input) {
    if (!input.isPrintAndPlay)
        return input.pdfBytes;
    ...
    for (let pageIndex = 0; pageIndex < Math.ceil(pdfEmbeddedPages.length / (columnCount * rowCount)); pageIndex++) {
